...
##        self.loadDir()
    ...
    def loadDir(self, dbg = True):
        if not dbg:
            ...
            s = r'D:\Libraries\Downloads\image dataset\resized'
##        if not os.path.isdir(s):
##            tkMessageBox.showerror("Error!", message = "The specified dir doesn't exist!")
##            return
        # get image list
        self.imageDir = s ##os.path.join(r'./Images', '%03d' %(self.category))
        #print self.imageDir 
        #print self.category
    ...
    def loadImage(self):
        ...
        print 'width: ' + str(max(self.tkimg.width(), 400))
        print 'height: ' + str(max(self.tkimg.height(), 400))
        self.mainPanel.config(width = max(self.tkimg.width(), 400), height = max(self.tkimg.height(), 400))
        self.mainPanel.create_image(0, 0, image = self.tkimg, anchor=NW)

        vsbar = Scrollbar(self.frame, orient="vertical", command=self.mainPanel.yview)
        vsbar.grid(row=1, column=1, sticky='e')
        # hsbar = Scrollbar(self.frame, orient="horizontal", command=self.mainPanel.xview)
        # hsbar.grid(row=1, column=1, sticky='sw')
        self.mainPanel.config(xscrollcommand=vsbar.set)#, yscrollcommand=vsbar.set)
        self.mainPanel.config(scrollregion=self.mainPanel.bbox('all'))
        # self.mainPanel.bind('<Configure>', self.on_configure)
        
        self.progLabel.config(text = "%04d/%04d" %(self.cur, self.total))

        # load labels
        self.clearBBox()
        self.imagename = os.path.splitext(imagepath)[0]
        #os.path.split(imagepath)[1].rsplit('.', 1)[0]
        #os.path.split(imagepath)[-1].split('.')[0] #should take the filename up to last '.' instead
        print os.path.split(imagepath)[1].rsplit('.', 1)[0]
